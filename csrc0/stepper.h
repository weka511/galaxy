...
class Stepper {
  ...
  private:
	 ...
	 */
	int _start_iteration(thread::id id);
	...
	int _thread_index(thread::id id) {
		int i=0;
		for ( const auto &p : _thread_status )
			if (p.first==id)
				return i;
	 ...
	 *   Controls behaviour
	 */
	map<thread::id, Status> 	_thread_status;
	 ...
	 */
	mutex 							_mtx_tree_has_been_built;

	condition_variable 			_cv_tree_has_been_built;
	
	/**
	 *  Used to let main know that threads have started
	 */
	mutex 							_mtx_starting;
	condition_variable 			_cv_starting;	

	/**
	 *  Used to let main know that threads are ending
	 */	
	mutex 							_mtx_finishing;
	condition_variable 			_cv_finishing;	
	 ...
	 */
	mutex 				_mtx_end_iter;
	condition_variable _cv_end_iter;
		
	/**
	 *  Used to build tree
	 */
	Node * (*_precondition)(vector<Particle*>);
	
	/**
	 *  Used to calculate acceleration of each particle
	 */
	void (*_get_acceleration)(int i, vector<Particle*> particles,Node * root);
	
	/**
	 *    Report results and decide whether to continue
	 */
	bool (*_shouldContinue)(vector<Particle*> particles,int iter);
